...
export async function generateMetadata({ params }: { params: Promise<{ locale: string }> }): Promise<Metadata> {
  ...
  return {
    title: t('rentTitle'),
    description: 'Browse properties for rent in the UAE. Find apartments, villas, and townhouses across Dubai, Abu Dhabi, and Sharjah.',
    openGraph: {
      title: t('rentTitle'),
      description: 'Browse properties for rent in the UAE. Find apartments, villas, and townhouses across Dubai, Abu Dhabi, and Sharjah.',
      type: 'website',
        ...
        <div className='gap-8 lg:grid lg:grid-cols-[280px_1fr]'>
          ...
          <main>
            <Suspense fallback={<PropertiesGridSkeleton />}>
              <RentContent searchParams={searchParams} params={params} />
            </Suspense>
          </main>
        </div>
      </div>
...
async function RentHeader({ params }: { params: Promise<{ locale: string }> }) {
  const { locale } = await params
  setRequestLocale(locale)
  const t = await getTranslations({ locale, namespace: 'search' })
  return <h1 className='text-3xl font-bold text-white'>{t('rentTitle')}</h1>
}

async function RentContent({
  searchParams,
  params,
}: {
  searchParams: Promise<{
